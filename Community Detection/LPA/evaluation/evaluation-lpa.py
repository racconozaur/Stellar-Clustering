...
print("\nLoading LPA results...")
load_start = time.time()
lpa_df = pd.read_csv('../lpa_tx_lcc_lpa_communities.csv')
print(f"Loaded {len(lpa_df)} node assignments ({time.time()-load_start:.2f}s)")

print("\nConverting to communities...")
convert_start = time.time()
communities_dict = lpa_df.groupby(lpa_df.columns[1])[lpa_df.columns[0]].apply(set).to_dict()
lpa_communities = list(communities_dict.values())
print(f"Found {len(lpa_communities)} communities ({time.time()-convert_start:.2f}s)")

sizes = [len(comm) for comm in lpa_communities]
results = {
    'method': 'LPA',
    'n_clusters': len(lpa_communities),
...
results['modularity'] = nx.algorithms.community.quality.modularity(G, lpa_communities)
print(f"  Modularity: {results['modularity']:.4f} ({time.time()-metric_start:.2f}s)")

metric_start = time.time()
results['coverage'] = nx.community.coverage(G, lpa_communities)
print(f"  Coverage: {results['coverage']:.4f} ({time.time()-metric_start:.2f}s)")

print(f"  Computing conductance for {len(lpa_communities)} communities...")
metric_start = time.time()
conductances = []
for i, comm in enumerate(lpa_communities):
    if len(comm) > 1:
        try:
            ...
            continue
    if (i + 1) % 100 == 0:
        print(f"    Processed {i+1}/{len(lpa_communities)} communities...")
...
pd.DataFrame([results]).to_csv('lpa_evaluation.csv', index=False)
print(f"\nResults saved to 'lpa_evaluation.csv'")
